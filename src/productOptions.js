...
const tableName = "ProductOptions";
...
router.get('/:id', function(req, res, next) {
    ...
    let query = `SELECT * FROM ${tableName} WHERE product_id = ${productId}`;
    connection.query(query, function(err,result,fields) {
        if(err){
            res.status(500).send(err);        
        }
...
router.post('/delete', function(req, res, next) {
    let item = req.body
    let query = `DELETE FROM ${tableName} WHERE product_id = ${item.product_id} `;
